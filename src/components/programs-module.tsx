...
export function ProgramsModule({ role }: Props) {
  ...
  const [createProgramType, setCreateProgramType] = useState<ProgramTypeValue | "">("");
  const [createProgramTypeFr, setCreateProgramTypeFr] = useState("");
  const [createProgramDuration, setCreateProgramDuration] = useState("");
  const [createProgramDurationFr, setCreateProgramDurationFr] = useState("");
  ...
  const [createDegreeLevelFr, setCreateDegreeLevelFr] = useState("");
  const [createFieldOfStudyFr, setCreateFieldOfStudyFr] = useState("");
  ...
  useEffect(() => {
    ...
    setEditProgramType(coerceProgramTypeValue(selected.programType));
    setEditProgramTypeFr(selected.programTypeFr ?? "");
    setEditProgramDuration(selected.programDuration ?? "");
    setEditProgramDurationFr(selected.programDurationFr ?? "");
    setEditDegreeLevel(coerceDegreeLevelValue(selected.degreeLevel));
    setEditFieldOfStudy(selected.fieldOfStudy ?? "");
    setEditDegreeLevelFr(selected.degreeLevelFr ?? "");
    setEditFieldOfStudyFr(selected.fieldOfStudyFr ?? "");
  ...
  const onCreateProgram = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      !createProgramType ||
      !createProgramTypeFr.trim() ||
      !createProgramDuration.trim() ||
      !createProgramDurationFr.trim() ||
      !createDegreeLevel ||
      !createDegreeLevelFr.trim() ||
      !createFieldOfStudy.trim() ||
      !createFieldOfStudyFr.trim() ||
      !(primaryLocalization.overview?.trim()) ||
      !createCurriculumList.length ||
      !createAdmissionList.length ||
      !createCareerList.length
    ) {
    ...
    try {
      const response = await fetch("/api/admin/programs", {
        ...
        body: JSON.stringify({
          ...
          programType: createProgramType || null,
          programTypeFr: createProgramTypeFr.trim() || null,
          programDuration: createProgramDuration.trim() || null,
          programDurationFr: createProgramDurationFr.trim() || null,
          degreeLevel: createDegreeLevel || null,
          degreeLevelFr: createDegreeLevelFr.trim() || null,
          fieldOfStudy: createFieldOfStudy.trim() || null,
          fieldOfStudyFr: createFieldOfStudyFr.trim() || null,
      ...
      setCreateAdmissionRequirements("");
              ...
              setCreateProgramTypeFr("");
              setCreateProgramDuration("");
              setCreateProgramDurationFr("");
              setCreateDegreeLevel("");
              setCreateFieldOfStudy("");
              setCreateDegreeLevelFr("");
      setCreateFieldOfStudyFr("");
      await loadData();
    } catch {
  ...
  const onUpdateProgram = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      !(primaryLocalization.overview?.trim()) ||
      !editCurriculumList.length ||
      !editAdmissionList.length ||
      !editCareerList.length
    ) {
      setError(t("error.requiredFields") || "All fields are required.");
          ...
          <section className="brand-card w-full max-w-3xl p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="grid gap-1.5">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setCreateProgramType(value);
                    setCreateProgramTypeFr(value ? getProgramTypeFrLabel(value) : "");
                  }}
                  required
                >
              ...
              <label className="grid gap-1.5">
                ...
                <select
                  className="brand-input"
                  value={createProgramTypeFr}
                  onChange={(e) => {
                    const value = e.currentTarget.value;
                    setCreateProgramTypeFr(value);
                    const derived = coerceProgramTypeValueFromFr(value);
                    if (derived) setCreateProgramType(derived);
                  }}
                  required
                >
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="grid gap-1.5">
                ...
                <input
                  className="brand-input"
                  value={createProgramDuration}
                  onChange={(e) => setCreateProgramDuration(e.currentTarget.value)}
              ...
              <label className="grid gap-1.5">
                ...
                <input
                  className="brand-input"
                  value={createProgramDurationFr}
                  onChange={(e) => setCreateProgramDurationFr(e.currentTarget.value)}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="grid gap-1.5">
                ...
                <select
                  className="brand-input"
                  value={createDegreeLevelFr}
                  onChange={(e) => setCreateDegreeLevelFr(e.currentTarget.value)}
                  required
                >
                  <option value="">Sélectionner le niveau d&apos;études</option>
                  {getDegreeLevelFrOptions(createDegreeLevelFr).map((opt) => (
                    <option key={opt} value={opt}>{opt}</option>
                  ))}
                </select>
              </label>
              <label className="grid gap-1.5">
                ...
                <input
                  ...
                  onChange={(e) => setCreateFieldOfStudy(e.currentTarget.value)}
                  maxLength={120}
                  required
                />
              </label>
              <label className="grid gap-1.5">
                ...
                <input
                  className="brand-input"
                  value={createFieldOfStudyFr}
                  onChange={(e) => setCreateFieldOfStudyFr(e.currentTarget.value)}
                  maxLength={120}
                  required
                />
              </label>
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  {selectedCreateCourses.map((course) => (
                    <span key={course.id} className="inline-flex items-center gap-2 rounded-full border border-[#9bbfed] bg-[#eff6ff] px-3 py-1 font-semibold text-[#0b3e81]">
                      {course.code} - {getCourseTitle(course, createSourceLanguage)}
                      <button
                        type="button"
                  ...
                  {createCourseSearch.trim() && (
                    <div className="absolute z-10 mt-2 w-full max-h-48 overflow-y-auto rounded-md border border-[#c6ddfa] bg-white p-2 shadow-lg">
                      {filteredCreateCourses.filter((c) => !createCourseIds.includes(c.id)).map((course) => (
                        ...
                        >
                          <span className="truncate">{course.code} - {getCourseTitle(course, createSourceLanguage)}</span>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {editCourseSearch.trim() && (
                    <div className="absolute z-10 mt-2 w-full max-h-48 overflow-y-auto rounded-md border border-[#c6ddfa] bg-white p-2 shadow-lg">
                      {filteredEditCourses.filter((c) => !editCourseIds.includes(c.id)).map((course) => (
                        ...
                        >
                          <span className="truncate">{course.code} - {getCourseTitle(course, editSourceLanguage)}</span>
                        </button>
                      ))}
                    </div>
                  )}
