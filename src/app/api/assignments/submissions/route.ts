...
export async function POST(request: NextRequest) {
  try {
    ...
    if (hasText && !config.allowedSubmissionTypes.includes("TEXT")) {
      return NextResponse.json({ error: "This assignment does not allow text submissions." }, { status: 400 });
    }
    if (hasFile && !config.allowedSubmissionTypes.includes("FILE")) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    >`
      SELECT s."id", s."assignmentId", s."studentId", s."latePenaltyPct", a."maxPoints", c."teacherId"
      FROM "AssignmentSubmission" s
    ...
    const rawScore = body.rawScore !== undefined ? parseScore(body.rawScore) : null;
    if (body.rawScore !== undefined && rawScore === null) {
    ...
    const cappedRaw = rawScore === null ? null : Math.min(maxPoints, rawScore);
    ...
    const status: SubmissionStatus = publish ? "PUBLISHED" : cappedRaw !== null || feedback ? "GRADED" : "SUBMITTED";
    ...
    await prisma.$executeRaw`
      ...
      SET
        ...
        "gradedById" = ${user.id},
        "gradedAt" = NOW(),
        "publishedAt" = CASE WHEN ${publish} THEN NOW() ELSE "publishedAt" END,
        "status" = ${status}
      WHERE "id" = ${submissionId}
    `;

    if (publish && finalScore !== null) {
